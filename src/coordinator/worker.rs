...
impl Worker
{
	...
	fn report(&self, res: Result<Message, Error>)
	{
		...
		if let Some(p) = promise {
			// If we can't send the error, it's because the receiving end has been dropped. We don't
			// care about that case.
			let _ = p.send(res);
		}
		else {
			// If we don't have a promise, things are going bad and are about to spiral
			// out-of-control... But there is nothing we can do about it short of aborting the
			// program. We don't do that since much of the other work might still be getting done
			// correctly.
			error!("AIO context attempting to update without a promise");
		}

		// This should never fail. If it does, we don't care because we're about to go out-of-scope.
